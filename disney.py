			...
			if Two_Factor != True:
				...
				if browser.find_elements_by_xpath('//*[@id="password__error"]'):
					...
					if 'Due to' in str(password_or_429_error):
						...
						print(" | 429 Too Many Requests. Please Change IP or Wait a while.\nEnding.\n")
						exit()
				if browser.find_elements_by_xpath('//*[@id="app_index"]/div[2]/div/div/h4'):
					email_error = browser.find_element_by_xpath('//*[@id="app_index"]/div[2]/div/div/h4').text
					if "We couldn't find" in str(email_error):
